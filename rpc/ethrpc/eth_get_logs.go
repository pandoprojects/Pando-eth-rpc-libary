...
func (e *EthRPCService) GetLogs(ctx context.Context, args EthGetLogsArgs) (result []EthGetLogsResult, err error) {
	...
	maxRetry := 5
	blocks := []*common.PandoGetBlockResultInner{}
...
func retrieveBlockByHash(blockhash tcommon.Hash, blocks *[](*common.PandoGetBlockResultInner), maxRetry int) (err error) {
	parse := func(jsonBytes []byte) (interface{}, error) {
		//logger.Infof("eth_getLogs.parse, jsonBytes: %v", string(jsonBytes))

		trpcResult := common.PandoGetBlockResult{PandoGetBlockResultInner: &common.PandoGetBlockResultInner{}}
	...
	var block common.PandoGetBlockResult
	for i := 0; i < maxRetry; i++ { // It might take some time for a tx to be finalized, retry a few times
		...
		if err == nil {
			block = resultIntf.(common.PandoGetBlockResult)
			break
		}

		logger.Warnf("eth_getLogs, pando.GetBlock returned error: %v", err)
		time.Sleep(blockInterval) // one block duration
	}

	if block.PandoGetBlockResultInner != nil {
		(*blocks) = append((*blocks), block.PandoGetBlockResultInner)
	}

	return nil
}

func retrieveBlocksByRange(fromBlock string, toBlock string, blocks *[](*common.PandoGetBlockResultInner), maxRetry int) (err error) {
	parse := func(jsonBytes []byte) (interface{}, error) {
		//logger.Infof("eth_getLogs.parse, jsonBytes: %v", string(jsonBytes))

		trpcResult := common.PandoGetBlocksResult{}

		var objList []json.RawMessage
		json.Unmarshal(jsonBytes, &objList)
		for _, blockJsonBytes := range objList {
			getBlockResult := common.PandoGetBlockResult{PandoGetBlockResultInner: &common.PandoGetBlockResultInner{}}
			...
			trpcResult = append(trpcResult, getBlockResult.PandoGetBlockResultInner)
		}
	...
	// if blockStart >= 2 {
	// 	blockStart -= 2 // Pando requires two consecutive committed blocks for finalization
	// }
	...
	for i := 0; i < maxRetry; i++ { // It might take some time for a tx to be finalized, retry a few times
		...
		rpcRes, rpcErr := client.Call("pando.GetBlocksByRange", trpc.GetBlocksByRangeArgs{Start: tcommon.JSONUint64(blockStart), End: tcommon.JSONUint64(blockEnd)})
		resultIntf, err := common.HandlePandoRPCResponse(rpcRes, rpcErr, parse)
		if err != nil {
			blocks = &[]*common.PandoGetBlockResultInner{}
			logger.Warnf("eth_getLogs, pando.GetBlocksByRange returned error: %v", err)
			time.Sleep(blockInterval) // one block duration
			continue
		}

		getBlocksRes := resultIntf.(common.PandoGetBlocksResult)
...
func extractLogs(addresses []tcommon.Address, topicsFilter [][]tcommon.Hash, filterByAddress bool, blocks [](*common.PandoGetBlockResultInner), result *([]EthGetLogsResult)) {
