...
func (e *EthRPCService) GetTransactionByHash(ctx context.Context, hashStr string) (result common.EthGetTransactionResult, err error) {
	...
	for i := 0; i < maxRetry; i++ { // It might take some time for a block to be finalized, retry a few times
		...
		if (pandoGetTransactionResult.BlockHash != tcommon.Hash{}) {
			break
		}
	...
	nativeTxHash := pandoGetTransactionResult.TxHash // need use native tx hash to find the tx index, instead of the ETH tx hash
	if pandoGetTransactionResult.Tx != nil {
		if types.TxType(pandoGetTransactionResult.Type) == types.TxSend {
			tx := pandoGetTransactionResult.Tx.(*types.SendTx)
		...
		if types.TxType(pandoGetTransactionResult.Type) == types.TxSmartContract {
			tx := pandoGetTransactionResult.Tx.(*types.SmartContractTx)
