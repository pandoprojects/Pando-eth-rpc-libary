...
func GetPandoRPCEndpoint() string {
	pandoRPCEndpoint := viper.GetString(CfgPandoRPCEndpoint)
	return pandoRPCEndpoint
}

func HandlePandoRPCResponse(rpcRes *rpcc.RPCResponse, rpcErr error, parse func(jsonBytes []byte) (interface{}, error)) (result interface{}, err error) {
	if rpcErr != nil {
		return nil, fmt.Errorf("failed to get pando RPC response: %v", rpcErr)
	}
	if rpcRes.Error != nil {
		return nil, fmt.Errorf("pando RPC returns an error: %v", rpcRes.Error)
	}

	var jsonBytes []byte
	jsonBytes, err = json.MarshalIndent(rpcRes.Result, "", "    ")
	if err != nil {
		return nil, fmt.Errorf("failed to parse pando RPC response: %v, %s", err, string(jsonBytes))
	}

	//logger.Infof("HandlePandoRPCResponse, jsonBytes: %v", string(jsonBytes))
	result, err = parse(jsonBytes)
	if err != nil {
		logger.Warnf("Failed to parse pando RPC response: %v, %s", err, string(jsonBytes))
	}
	return
}
...
func GenerateSctx(arg EthSmartContractArgObj) (result *types.SmartContractTx, err error) {
	...
	from := types.TxInput{
		...
		Coins: types.Coins{
			...
			//PTXWei: new(big.Int).SetUint64(Str2hex2unit(arg.Value)),
			PTXWei: HexStrToBigInt(arg.Value),
		},
		Sequence: sequence,
	}
...
func GetSeqByAddress(address tcommon.Address) (sequence uint64, err error) {
	client := rpcc.NewRPCClient(GetPandoRPCEndpoint())

	rpcRes, rpcErr := client.Call("pando.GetAccount", trpc.GetAccountArgs{Address: address.String()})
	...
	resultIntf, err := HandlePandoRPCResponse(rpcRes, rpcErr, parse)
